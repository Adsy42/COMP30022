...
describe("ChoiceGroup (single)", () => {
  test("renders with correct roles and option order", () => {
    ...
    expect(chips.map(c => (c as HTMLButtonElement).textContent)).toEqual(OPTIONS);
  });
...
describe("ChoiceGroup (multi)", () => {
  ...
  test("toggles chips on/off and aggregates selection", async () => {
    ...
    expect(onChange.mock.calls.some(call => JSON.stringify(call[0]) === JSON.stringify(["Pre-Award"]))).toBe(true);

...
export function AddQuestionModal({
  ...
  const handleFollowUpAdd = (
    optionIndex: number,
    followUpQuestion: Omit<FormQuestion, 'id' | 'order'>
  ) => {
    setFormData(prev => ({
      ...prev,
                ...
                <select
                  ...
                  onChange={e =>
                    setFormData(prev => ({
                      ...
                      options:
                        e.target.value === 'text' ? undefined : prev.options,
                    }))
                  }
                  className="w-full px-3 py-2 border border-gray-200 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500"
                >
                              ...
                              {option.followUps.map(followUp => (
                                ...
                                >
                                  <div>
                                    ...
                                    <div>
                                      Type: {questionTypes[followUp.type]}
                                    </div>
                                  </div>
                                  <div className="invisible group-hover/followup:visible flex gap-1">
                                    ...
                                    <button
                                      type="button"
                                      onClick={() =>
                                        handleDeleteFollowUp(index, followUp.id)
                                      }
      ...
      {isFollowUpModalOpen && (
        <AddQuestionModal
          ...
          onAdd={followUpQuestion => {
            const optionIndex =
              formData.options?.findIndex(opt => opt === selectedOption) ?? -1
            if (optionIndex !== -1) {

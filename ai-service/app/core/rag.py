...
class RAGServicePinecone:
    ...
    def _initialize_pinecone(self):
        ...
        try:
            ...
            if self.index_name not in [index.name for index in self.pc.list_indexes()]:
                print(f"Index {self.index_name} not found. Please create it in the Pinecone console first.")
                print(f"Required settings: dimension=384, metric=cosine, type=dense")
    ...
    def _initialize_llm(self):
        ...
        try:
            ...
            if not hf_token:
                print("Warning: HF_TOKEN not found. Using public API (may have rate limits)")
            else:
    ...
    def add_documents(self, documents: List[LangchainDocument], resource_id: str):
        ...
        try:
            ...
            print(f"Successfully added {len(documents)} documents to Pinecone for resource {resource_id}")
    ...
    def add_faq(self, faq_documents: List[LangchainDocument], resource_id: str):
        ...
        try:
            ...
            print(f"Successfully added {len(faq_documents)} FAQ documents to Pinecone for resource {resource_id}")
            
        except Exception as e:
            raise Exception(f"Error adding FAQ: {str(e)}")
    
    def query(self, question: str, max_results: int = 5, include_documents: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            prompt = f"""
            You are a helpful assistant that can answer legal contract questions based on resources provided by
            ...
            raise Exception(f"Error processing query: {str(e)} - Details: {error_details}")
    ...
    def delete_resource(self, resource_id: str):
        ...
        try:
            ...
            if ids_to_delete:
                ...
                print(f"Deleted {len(ids_to_delete)} documents for resource {resource_id}")
            else:
